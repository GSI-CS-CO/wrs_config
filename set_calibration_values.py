...
port_cnt = 18
inst_cnt = 1
target_hw_ver = "3.4"  # Default hardware version
...
def set(input_file, calib_file, target_hw_ver="3.4"):
    # Check input arguments
    if is_file_missing(input_file):
    ...
    output_file = f"{input_file}_v{target_hw_ver}"
    ...
    for port in range(1, port_cnt + 1):
        idx = port - 1
        port_values = calib_values[idx].split(':')
    ...
    # Write updated configuration to output file
...
if __name__ == '__main__':
    ...
    set(args.input, args.calibration, args.target)
